...
CHATBOT_TITLE = "Republic TV Chatbot"
CHATBOT_DESCRIPTION = "This chatbot maintains conversation context and searches for information when it does not know and Talks like Arnab Goswami"
...
def summarize_conversation(history):
    ...
    summarize_prompt = [
        SystemMessage(content="You are a helpful assistant. Summarize the following conversation into a concise paragraph capturing the key information and facts discussed. Focus only on factual information that would be useful to remember for future conversations."),
        HumanMessage(content=f"Here's the conversation to summarize:\n\n{conversation_text}")
    ]
    ...
    try:
        ...
        return f"Conversation on {datetime.now().strftime('%Y-%m-%d %H:%M')} (failed to summarize)"
...
def chat(message, history, debug_output=None):
    ...
    if memory_context and DEBUG_MODE:
        debug_logs.append(f"**Memory Context**\n{memory_context}")

    # Add system message first with memory context
    system_prompt = (
        "You are an assistant who talks like Arnab Goswami and use his style in your response. If you don't know the answer to a question, "
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Memory"):
                memory_display = gr.Markdown("Memory will be displayed here")
        ...
        def clear_and_save_memory(history):
            ...
            return [], "Debug information will appear here", memory_text
